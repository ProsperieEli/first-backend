...
const { insert, getAll, getById } = require('../lib/models/Order');
...
describe('refactory routes', () => {
  ...
  it('should be able to list an order by id', async () => {
    const order = await Order.insert({ product: 'Wood', quantity: 1 });
  ...
  it('should be able to list orders', async () => {
    const getAll = await Order.insert({ product: 'wood', quantity: 1 });
    const res = await request(app).get('/api/v1/orders');

    expect(res.body).toEqual([getAll]);
  });
